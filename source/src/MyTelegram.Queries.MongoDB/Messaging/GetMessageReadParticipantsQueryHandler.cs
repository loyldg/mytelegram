    ...
    IReadOnlyCollection<IReadingHistoryReadModel>>
{
    ...
    public async Task<IReadOnlyCollection<IReadingHistoryReadModel>> ExecuteQueryAsync(
        GetMessageReadParticipantsQuery query,
        CancellationToken cancellationToken)
    {
        var cursor = await _store.FindAsync(p => p.TargetPeerId == query.TargetPeerId && p.MessageId == query.MessageId,
                cancellationToken: cancellationToken)
            ;

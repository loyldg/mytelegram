...
public class HandlerHelper : IHandlerHelper
{
        ...
        [NotNullWhen(true)] out IObjectHandler? handler)
    {
        ...
        if (!isInitOk) _semaphoreSlim.Wait();

        if (Handlers.TryGetValue(objectId, out handler))
        {
            if (!isInitOk) _semaphoreSlim.Release();
    ...
    {
        if (_isLoadingHandlers || _isInitOk) return;
    ...
    {
        if (!Handlers.ContainsKey(objectId)) Handlers.TryAdd(objectId, handler);
    }
}
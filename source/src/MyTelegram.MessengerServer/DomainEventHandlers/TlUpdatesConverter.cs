...
public class TlUpdatesConverter : ITlUpdatesConverter
{
    ...
    public IUpdates ToUpdates(ReceiveInboxMessageCompletedEvent aggregateEvent)
    {
        ...
        if (item.SendMessageType == SendMessageType.MessageService)
        {
            ...
            {
                throw new ArgumentNullException(nameof(aggregateEvent), "MessageActionData can not be null for service message");
            }
    ...
    public IUpdates ToChannelMessageUpdates(SendOutboxMessageCompletedEvent aggregateEvent)
    {
        ...
        var updateNewChannelMessage =
            new TUpdateNewChannelMessage { Message = _messageConverter.ToMessage(aggregateEvent.MessageItem, selfUserId), Pts = aggregateEvent.Pts, PtsCount = 1 };
    ...
    {
        var update = new TUpdateEditMessage { Message = _messageConverter.ToMessage(aggregateEvent), Pts = aggregateEvent.Pts, PtsCount = 1 };
    ...
    private IUpdates ToInboxForwardMessageUpdates(MessageItem aggregateEvent, int pts)
    {
        ...
        SendOutboxMessageCompletedEvent aggregateEvent)
    {
        ...
        var chat = _chatConverter.ToChat(eventData.ChatId, eventData.Title, eventData.Date, eventData.MemberUidList.Count);
        ...
        bool createUpdatesForSelf)
    {
        ...
        var channel = _chatConverter.ToChannel(channelReadModel,
            null,
            createUpdatesForSelf ? item.SenderPeer.PeerId : 0, false);

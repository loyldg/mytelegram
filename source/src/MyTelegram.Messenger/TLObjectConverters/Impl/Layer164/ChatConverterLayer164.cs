...
public class ChatConverterLayer164 : ChatConverterBase, IChatConverterLayer164
{
    ...
    public ChatConverterLayer164(IObjectMapper objectMapper,
    ...
    )
    {
        ...
        if (channelFullReadModel.RecentRequesters?.Count > 0 && channelReadModel.AdminList.Any(p => p.UserId == selfUserId))
        {
            channelFull.RequestsPending = channelFullReadModel.RequestsPending;
            channelFull.RecentRequesters = new(channelFullReadModel.RecentRequesters);
        }
    ...
    public Schema.Channels.IChannelParticipant ToChannelParticipantLayerN(
        long selfUserId,
        IChannelReadModel channelReadModel,
        IPhotoReadModel? photoReadModel,
        IChannelMemberReadModel channelMemberReadModel,
    ...
    )
    {
        ...
        return new Schema.Channels.LayerN.TChannelParticipant() { Participant = participant, Users = new TVector<IUser>(user) };
    }
    ...
    {
        var participants = ToChannelParticipantsCore(selfUserId, channelReadModel, chatAdminReadModels, channelMemberReadModels);
    ...
    {
        var channel = ToChannel(selfUserId, channelReadModel, photoReadModel, channelMemberReadModel);
        var chat = migratedFromChatReadModel == null ? null : ToChat(selfUserId, migratedFromChatReadModel, photoReadModel);
    ...
    protected virtual Schema.IChannelParticipant ToChatParticipantAdmin(long selfUserId, IChatAdminReadModel chatAdminReadModel)
    {

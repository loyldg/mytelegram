...
{
    private static readonly byte[] KeyTypeBytes = "MyTelegram_Message"u8.ToArray();
    ...
    public int Encrypt(int keyId, ReadOnlySpan<byte> masterKey, long ownerPeerId, string plainText, Span<byte> cipherTextWithNonceAndTag)
    {
        //// 4:keyId 12:nonce 16:tag messageKey:32 8:salt
        var length = cipherTextWithNonceAndTag.Length;// 4 + plainText.Length * 4 + 12 + 16 + 32 + 8;
        ...
        var span = cipherTextWithNonceAndTag;

...
public class EventBusRabbitMq : IEventBus, IDisposable
{
    ...
    private readonly IRabbitMqPersistentConnection _consumerConnection;
    private readonly IRabbitMqPersistentConnection _publishConnection;
    ...
    public EventBusRabbitMq(IRabbitMqPersistentConnection persistentConnection,
        ...
        IRabbitMqSerializer rabbitMqSerializer, IRabbitMqConnectionFactory rabbitMqConnectionFactory)
    {
        ...
        _consumerConnection = _rabbitMqConnectionFactory.CreateConnection();
        _publishConnection = _rabbitMqConnectionFactory.CreateConnection();
    ...
    private IModel CreateConsumerChannel()
    {
        ...
        channel.ContinuationTimeout = TimeSpan.FromSeconds(3);

        channel.CallbackException += (sender,
            ea) =>
        {
            _logger.LogWarning(ea.Exception, "[Callback exception]Recreating RabbitMQ consumer channel");
        ...
        {
            _logger.LogWarning("Rabbit consumer channel shutdown,reason:{Reason}", e.Cause);
            _logger.LogWarning("[ModelShutdown]Recreating RabbitMQ consumer channel");
            DisposeChannel();
            _consumerChannel = CreateConsumerChannel();
            StartBasicConsume();
        };
    ...
    private void DisposeChannel()
    {
        ...
        {
            _logger.LogWarning(ex, "Dispose RabbitMQ channel failed");
        }
    }
    ...
    private Task PublishAsync(Type eventDataType,
        byte[] eventData)
    {
        if (!_publishConnection.IsConnected)
        {
            _publishConnection.TryConnect();
        }

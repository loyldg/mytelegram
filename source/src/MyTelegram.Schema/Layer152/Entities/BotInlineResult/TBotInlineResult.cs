// <auto-generated/>
// ReSharper disable All

namespace MyTelegram.Schema;


///<summary>
///See <a href="https://core.telegram.org/constructor/botInlineResult" />
///</summary>
[TlObject(0x11965f3a)]
public sealed class TBotInlineResult : IBotInlineResult
{
    public uint ConstructorId => 0x11965f3a;
    public BitArray Flags { get; set; } = new BitArray(32);
    public string Id { get; set; }
    public string Type { get; set; }
    public string? Title { get; set; }
    public string? Description { get; set; }
    public string? Url { get; set; }

    ///<summary>
    ///See <a href="https://core.telegram.org/type/WebDocument" />
    ///</summary>
    public MyTelegram.Schema.IWebDocument? Thumb { get; set; }

    ///<summary>
    ///See <a href="https://core.telegram.org/type/WebDocument" />
    ///</summary>
    public MyTelegram.Schema.IWebDocument? Content { get; set; }

    ///<summary>
    ///See <a href="https://core.telegram.org/type/BotInlineMessage" />
    ///</summary>
    public MyTelegram.Schema.IBotInlineMessage SendMessage { get; set; }

    public void ComputeFlag()
    {
        if (Title != null) { Flags[1] = true; }
        if (Description != null) { Flags[2] = true; }
        if (Url != null) { Flags[3] = true; }
        if (Thumb != null) { Flags[4] = true; }
        if (Content != null) { Flags[5] = true; }

    }

    public void Serialize(BinaryWriter bw)
    {
        ComputeFlag();
        bw.Write(ConstructorId);
        bw.Serialize(Flags);
        bw.Serialize(Id);
        bw.Serialize(Type);
        if (Flags[1]) { bw.Serialize(Title); }
        if (Flags[2]) { bw.Serialize(Description); }
        if (Flags[3]) { bw.Serialize(Url); }
        if (Flags[4]) { Thumb.Serialize(bw); }
        if (Flags[5]) { Content.Serialize(bw); }
        SendMessage.Serialize(bw);
    }

    public void Deserialize(BinaryReader br)
    {
        Flags = br.Deserialize<BitArray>();
        Id = br.Deserialize<string>();
        Type = br.Deserialize<string>();
        if (Flags[1]) { Title = br.Deserialize<string>(); }
        if (Flags[2]) { Description = br.Deserialize<string>(); }
        if (Flags[3]) { Url = br.Deserialize<string>(); }
        if (Flags[4]) { Thumb = br.Deserialize<MyTelegram.Schema.IWebDocument>(); }
        if (Flags[5]) { Content = br.Deserialize<MyTelegram.Schema.IWebDocument>(); }
        SendMessage = br.Deserialize<MyTelegram.Schema.IBotInlineMessage>();
    }
}
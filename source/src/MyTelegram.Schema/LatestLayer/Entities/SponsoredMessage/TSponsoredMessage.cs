...
{
    public uint ConstructorId => 0xed5383f7;
    ///<summary>
    ...
    public void ComputeFlag()
    {
        ...
        if (App != null) { Flags[10] = true; }
        if (Entities?.Count > 0) { Flags[1] = true; }
        if (ButtonText != null) { Flags[11] = true; }
    ...
    public void Serialize(IBufferWriter<byte> writer)
    {
        ...
        if (Flags[10]) { writer.Write(App); }
        writer.Write(Message);
        if (Flags[1]) { writer.Write(Entities); }
        if (Flags[11]) { writer.Write(ButtonText); }
    ...
    public void Deserialize(ref SequenceReader<byte> reader)
    {
        ...
        if (Flags[10]) { App = reader.Read<MyTelegram.Schema.IBotApp>(); }
        Message = reader.ReadString();
        if (Flags[1]) { Entities = reader.Read<TVector<MyTelegram.Schema.IMessageEntity>>(); }
        if (Flags[11]) { ButtonText = reader.ReadString(); }
